...
### 📋 免責事項
**本プログラムの利用により生じた損害について、開発者は一切の責任を負いません。**

このプロジェクトは実験的・教育的な目的で開発されており、商用利用や本番環境での使用を前提としていません。利用者は自己責任において本プログラムを使用してください。

### ⚠️ 本番環境での使用に関する注意

#### 1. 本番ネットワークでのテスト未実施
- **現状**: 本プロジェクトはテストネット（Polygon Amoy、Ethereum Sepolia等）でのみテストされています
- **本番ネットワーク**: Polygon Mainnet、Ethereum Mainnetでの動作検証は行っていません
- **リスク**: 実際の資産を扱う環境では予期しない挙動が発生する可能性があります

#### 2. 必要な設定とセキュリティ対策
本番環境で使用する場合は、以下の設定・対策が必須です：

**RPC設定:**
- ⚠️ 公開RPCエンドポイントではなく、APIキー付きプロバイダ（Infura、Alchemy、QuickNode等）を使用してください
- 環境変数 `VITE_SEPOLIA_RPC`、`VITE_POLYGON_RPC` 等に適切なエンドポイントを設定してください

**WalletConnect設定:**
- ⚠️ [WalletConnect Cloud](https://cloud.walletconnect.com/)で独自のプロジェクトIDを取得してください
- 環境変数 `VITE_WALLETCONNECT_PROJECT_ID` にあなたのプロジェクトIDを設定してください
- デモ用のプロジェクトIDは本番環境では使用しないでください

**セキュリティ監査:**
- スマートコントラクトとの連携部分について、専門機関による監査を受けることを強く推奨します
- フロントラン攻撃、再入攻撃等の脆弱性チェックを実施してください

**法的コンプライアンス:**
- 暗号資産交換業に関する法規制を確認してください
- 資金決済法、金融商品取引法等の関連法令を遵守してください

#### 3. PWA機能のテスト未実施
- **PWAインストール機能**: 基本実装は完了していますが、十分なテストは行われていません
- **オフライン動作**: Service Workerの動作保証はありません
- **プッシュ通知**: 実装されていない、または動作未検証です
- **推奨**: 本番環境で使用する前に、各種デバイス・ブラウザでの動作確認を行ってください

#### 4. ビルド環境でのテスト未実施
- **現状**: 開発サーバー（`npm run dev`）でのみテストされています
- **本番ビルド**: `npm run build` でビルドした静的ファイルの動作検証は行っていません
- **推奨**: 
  - 本番デプロイ前に `npm run build` → `npm run preview` で動作確認を実施してください
  - Vercel/Netlify等のホスティングサービスでのデプロイ前テストを推奨します
  - プロキシ設定やCORS設定が本番環境で正しく機能することを確認してください

### 🔒 セキュリティ上の推奨事項
- プライベートキーや秘密鍵は絶対にリポジトリにコミットしないでください
- `.env` ファイルは `.gitignore` に含まれていることを確認してください
- APIキー、プロジェクトID等の機密情報は環境変数で管理してください
- 本番環境では、レート制限・アクセス制御等の保護機能を実装してください
...
**Ambire Wallet未対応について:**
- 現在のガスレス決済実装は、Ambire Wallet SDKを使用している関係上、Ambire Walletでは動作しません
- Ambire Walletのアカウント抽象化機能とは別のガスレス実装方式を採用しているため、互換性がありません

**ガスレス決済の動作条件:**
- 3つのガスレス実行モード（メタトランザクション、ペイマスター、リレーヤー）での動作を確認していますが、これはテスト環境での検証結果です
- 使用するネットワークやトークンによっては、ガスレス決済ができない可能性があります
- 本実装のガスレス決済機能については、客観的な第三者による検証が必要です
- 本番環境での利用前に、十分なテストと検証を実施してください

**3つのガスレス実行モード - テスト環境での動作確認済み**
